...
const CategoryModal = ({ isOpen, onClose, mode, category, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "create") {
        await TestsService.createCategory(formData);
      } else {
        await TestsService.updateCategory(category.id, formData);
      }
      onSuccess();
      onClose();
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-8 flex justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium shadow-lg shadow-blue-500/30 transition-all transform hover:scale-105"
            >
              {loading ? (
